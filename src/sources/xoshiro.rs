...
pub fn xoshiro256_star_star(seed: Option<&str>) -> Result<Box<dyn Source>, crate::core::SourceError> {
    let (rng, seed_str) = seeded_rng::<Xoshiro256StarStar>(seed)?;
    Ok(Box::new(
        RngSource::new(rng, "xoshiro256**", SourceKind::Prng).with_seed(seed_str.unwrap_or_default()),
    ))
}

/// xoshiro256++ — slightly faster sibling.
pub fn xoshiro256_plus_plus(seed: Option<&str>) -> Result<Box<dyn Source>, crate::core::SourceError> {
